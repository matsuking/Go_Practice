...
func slice_map_for() {
	...
	s6 := s5[1:3]
	// もとのs5の配列も値が書き換わってしまう（メモリー領域の共有）
	s6[1] = 10
	fmt.Printf("s5: %v %v %v\n", s5, len(s5), cap(s5))
	fmt.Printf("s6: %v %v %v\n", s6, len(s6), cap(s6))
	...
	// vは値が入り、okは存在するかのtrue, falseが入ってくる
	v, ok := m2["A"]
	fmt.Printf("%v %v \n", v, ok)
